...
public final class NumbersUtils {
    ...
    private static final float TWO_POW_23_F = (float) (1<<23);

    private static final double TWO_POW_52 = (double) (1L<<52);
     ...
     * @throws ArithmeticException if the specified value is not in byte range.
     */
    public static byte asByte(short a) {
        final byte ret = (byte) a;
        if (ret != a) {
     ...
     * @throws ArithmeticException if the specified value is not in byte range.
     */
    public static byte asByte(long a) {
        final byte ret = (byte) a;
        if (ret != a) {
            throw new ArithmeticException("overflow: " + a);
        }
    ...
    /**
     * @param a An int value.
     * @return The specified value as short.
     * @throws ArithmeticException if the specified value is not in short range.
     */
    public static short asShort(int a) {
        final short ret = (short) a;
        if (ret != a) {
            throw new ArithmeticException("overflow: " + a);
        }
        return ret;
     ...
     * @throws ArithmeticException if the specified value is not in short range.
     */
    public static short asShort(long a) {
        final short ret = (short) a;
        if (ret != a) {
            throw new ArithmeticException("overflow: " + a);
        }
     ...
     * @throws ArithmeticException if the specified value is not in int range.
     */
    ...
    /**
     * @param a A short value.
     * @return The closest byte value.
     */
    public static byte toByte(short a) {
        byte ret = (byte) a;
        if (ret != a) {
            ret = (a < 0) ? Byte.MIN_VALUE : Byte.MAX_VALUE;
        }
        return ret;
    }
    ...
    /**
     * @param a An int value.
     * @return The closest short value.
     */
    public static short toShort(int a) {
        short ret = (short) a;
        if (ret != a) {
            ret = (a < 0) ? Short.MIN_VALUE : Short.MAX_VALUE;
        }
        return ret;
    }
    ...
    /**
     * Compared to Integer.toString(int,int), this method
     * uses upper case letters, and should be faster.
     * 
     * @param value An int value.
     * @param radix A radix in [2,36].
    ...
    /**
     * Compared to Long.toString(long,int), this method
     * uses upper case letters, and should be faster.
     * 
     * @param value A long value.
     * @param radix A radix in [2,36].
     ...
     * @param value An int value.
     * @param radix A radix in [2,36].
     * @param paddingUpTo Number of digits (sign excluded) up to which
     *        left-padding with zeros is done. Can be any value.
     * @return String representation of the specified value in the specified radix.
     * @throws IllegalArgumentException if the specified radix is out of range.
     */
    public static String toString(int value, int radix, int paddingUpTo) {
        if (radix == 10) {
            ...
            // which makes things faster even with eventual padding.
            return leftPaddedWithZero(Integer.toString(value), paddingUpTo);
        }
        ...
        int charIndex = nbrOfChars;

        final boolean radixIsPowerOfTwo = ((radix & (radix-1)) == 0);
        // Not allowing Integer.MIN_VALUE, which can't be negated.
        if (radixIsPowerOfTwo && (negValue != Integer.MIN_VALUE)) {
            // Optimization : bit ops instead of % and /.
            final int mask = radix-1;
            final int divShift = DIV_SHIFT_BY_RADIX[radix];
            while (negValue <= -radix) {
                chars[--charIndex] = CHAR_BY_DIGIT[(int)((-negValue) & mask)];
                negValue = -((-negValue) >> divShift);
            }
        } else {
            while (negValue <= -radix) {
                chars[--charIndex] = CHAR_BY_DIGIT[(int)(-(negValue % radix))];
                negValue /= radix;
            }
        }
        chars[--charIndex] = CHAR_BY_DIGIT[(int)(-negValue)];

        while (charIndex > signSize) {
            chars[--charIndex] = '0';
        }
    ...
    public static String toString(long value, int radix, int paddingUpTo) {
        if (radix == 10) {
            // Using JDK's optimized algorithm,
            // which makes things faster even with eventual padding.
            return leftPaddedWithZero(Long.toString(value), paddingUpTo);
        }
        ...
        final boolean radixIsPowerOfTwo = ((radix & (radix-1)) == 0);
        // Not allowing Long.MIN_VALUE, which can't be negated.
    ...
    /**
     * For use for special cases implementation,
     * or as reference for tests and benches.
     * 
     * Preserves minus sign on the left if any.
     * 
     * @param str String of a mathematical integer.
     * @param paddingUpTo Can be any value.
     * @return The specified number with at least "paddingUpTo" digits.
     */
    static String leftPaddedWithZero(String str, int paddingUpTo) {
        
        if (paddingUpTo <= 1) {
            return str;
        }
        
        final boolean isNeg = str.startsWith("-");
        ...
        if (paddingUpTo > digitCount) {
            final int signSize = (isNeg ? 1 : 0);
            final StringBuilder sb = new StringBuilder(signSize + paddingUpTo);
            
            if (isNeg) {
                sb.append('-');
            }
            
            final int paddingSize = paddingUpTo - digitCount;
            for (int i = 0; i < paddingSize; i++) {
                sb.append('0');
            }
            
            sb.append(str, signSize, str.length());

...
import net.jolikit.internal.nodepto.commonslogging.Log;
import net.jolikit.internal.nodepto.commonslogging.LogFactory;
...
public class CmapSubtable
{
    ...
    /**
     * TODO fontbox Added this not to uselessly loop out of range.
     */
    public int getMaxGid() {
        return this.glyphIdToCharacterCode.length - 1;
    }

    /**
     * TODO fontbox Added this as a faster version of getCharCodes(gid),
     * without order (we don't care).
     * 
     * @param gid Must be in range.
     */
    public int[] getCharCodesArr(int gid)
    {
        ...
        if (code == Integer.MIN_VALUE)
        {
            ...
            if (mappedValues != null)
            {
                codeArr = new int[mappedValues.size()];
                final int size = mappedValues.size();
                for (int i = 0; i < size; i++) {
                    final Integer value = mappedValues.get(i);
                    codeArr[i] = value.intValue();
                }
            }
        }
        else
        {
            codeArr = new int[]{code};
        }
        return codeArr;
    }
